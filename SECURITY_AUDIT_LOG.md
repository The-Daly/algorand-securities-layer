...
| REV-002 | 2026-07-19 | `claude/bootstrap-repo` (PR #1) | Pre-merge hardening: `project-audit` CI (Node 22, `contents: read`, pinned Action SHAs), `.github/CODEOWNERS`, `main` branch protection, SEC-008 mitigation path | Complete | Pending | Pending | Blocked | **Builder self-review only.** `npm run check` passes; branch protection + CODEOWNERS + CI added; release gates still fail by design; SEC-008 stays Open. Pass 2 (adversarial) and Pass 3 (release) NOT performed. |
...
| SEC-008 | 2026-07-19 | Release automation | Medium | The release gate reads a plain-text "Result" cell and finding statuses from Markdown; nothing cryptographically binds a "Passed"/"Closed" claim to the reviewed commit or to distinct reviewers, so the gate can be bypassed by editing text. | Technical lead | Before Phase 1 | Open | Builder self-review (REV-001) observation. Accepted as a **procedural** control for now (do not over-engineer custom attestations). Mitigation path: protected `main` branch, required independent review, CODEOWNERS, CI bound to the reviewed commit (`project-audit`), and an independent professional review before real assets or MainNet. Branch protection + CODEOWNERS + CI added in REV-002; finding stays **Open** until CI is a required check on `main` and an independent Pass 2/3 is recorded. |
